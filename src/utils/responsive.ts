...
export const scale = (sizePhone: number, sizeTablet?: number) => {
  const safeAreaWidthPadding = initialWindowSafeAreaInsets.left + initialWindowSafeAreaInsets.right;
  ...
  if (isTablet() && sizeTablet) {
    return Math.round(
      PixelRatio.roundToNearestPixel(
        (windowHeight / guidelineBaseWidthLandscape) * (sizeTablet || sizePhone),
      ),
    );
  }
  return Math.round(
